...
> - Claude Code：Anthropic memory 官方文档，核验于 2026-06-13：`https://code.claude.com/docs/en/memory`
...
Codex manual 把 memories 描述为可选功能：默认关闭，开启后 Codex 可把早期 thread 中有用的上下文转成本地 memory files，默认存放在 Codex home 下的 `~/.codex/memories/`。文档同时强调团队必须遵守的规则应放在 `AGENTS.md` 或 checked-in docs，memories 是 helpful local recall layer，不是强制规则来源（官方文档：`https://developers.openai.com/codex/codex-manual.md` 的 `Memories`、`Custom instructions with AGENTS.md`）。

Codex 的 memory 还带有生成边界：它会跳过 active 或 short-lived sessions，后台更新而不是每个 thread 结束立刻写，且可配置是否使用已有 memories、是否把当前 thread 作为 memory generation input、是否在使用 MCP/web search 等 external context 时禁用 memory generation（官方文档：`https://developers.openai.com/codex/codex-manual.md` 的 `Memories`）。

Claude Code 的官方 memory 页把跨 session 知识分成 `CLAUDE.md` 和 auto memory：`CLAUDE.md` 是用户写的 persistent instructions，auto memory 是 Claude 根据纠正和偏好写的 notes；两者都会作为 context 载入，但不是 enforcement configuration。要硬性阻止动作，文档建议用 `PreToolUse` hook，而不是只靠 memory（官方文档：`https://code.claude.com/docs/en/memory`）。

因此本章加入 Codex/Claude Code 时要保持一个边界：repo guidance、`AGENTS.md`/`CLAUDE.md`、skills、MCP data source 和 memories 都可能影响上下文，但不是同一种“长期记忆”。规则/约束放 guidance 或 hooks，过程放 skills，外部实时数据走 MCP，用户偏好和稳定事实才适合 memory。
...
| --- | --- | --- | --- |
| 记忆定位 | memories 默认关闭，开启后生成 `~/.codex/memories/` 下的本地记忆文件。 | `CLAUDE.md` 是 persistent instruction，auto memory 是偏好/纠正 notes。 | memory 是 recall layer，不应替代强制规则或项目文档。 |
| 规则边界 | required team guidance 应放 `AGENTS.md` 或 checked-in docs。 | 阻止动作要用 hook/permission，不能只靠 memory。 | guidance、memory、hooks 必须分层，否则安全策略会被误写成提示词。 |
| 生成时机 | 后台生成，跳过 active/short-lived sessions，可因 external context 设置禁用。 | auto memory 从用户纠正和偏好中学习，作为后续 context。 | memory 写入应有 eligibility、secret redaction 和外部上下文污染控制。 |
| 与 MCP/skills | MCP 提供实时外部数据，skills 提供过程知识。 | skills/subagents/hooks 与 memory 是不同扩展面。 | 外部数据源和过程技能不要塞进长期偏好记忆。 |
...
9. Codex 读 manual 的 `Memories`、`Custom instructions with AGENTS.md`、`Customization`，确认 memory、repo guidance、skills 和 MCP 的分工。
10. Claude Code 读 `https://code.claude.com/docs/en/memory`，确认 `CLAUDE.md`、auto memory、rules 和 hook enforcement 的边界。
...
- `GuidanceMemoryBoundary`：把强制规则、项目指导、过程技能、外部实时上下文和可召回偏好分开；Codex 的 `AGENTS.md`/memories/skills/MCP 和 Claude Code 的 `CLAUDE.md`/auto memory/hook 分工都支持这个抽象。
...
- 不要把 Codex memories 当成必须遵守的团队规则；官方文档要求 required team guidance 放在 `AGENTS.md` 或 checked-in docs。
- 不要把 Claude Code auto memory 当成安全策略；官方文档说 `CLAUDE.md` 和 auto memory 是 context，不是 enforcement configuration，阻止动作要用 hook/permission。
